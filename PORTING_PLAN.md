...
`js/zones.js` lines 1666-1691). Every other zone/system is still not
ported, and that includes a SEPARATE, still-open category: the remaining
hand-authored zones (the Forsaken Chapel, Ashgrove Hollow, the Caravan
Zone, the Homestead), each its own function in `js/zones.js` that would
need its own transcription the same way the zones above were done --
NOT covered by `buildProceduralZone()` (the 4 `ZONE_CONFIGS` biomes) or
by `buildDungeonMap()` (the dungeon generator). Build with:
