...
determinism. **This same pass also fixed a real portal-graph bug**: reading
...
one remaining hand-authored zone (the Homestead, plus the Caravan Zone
itself, `makeCaravanZoneMap()`), each its own function in `js/zones.js` that
would need its own transcription the same way the zones above were done --
NOT covered by `buildProceduralZone()` (the 4 `ZONE_CONFIGS` biomes) or by
