...
door tiles. Every other zone/system is still not ported, and that includes
a SEPARATE, still-open category: the remaining hand-authored zones (the
Forsaken Chapel, Ashgrove Hollow, the Caravan Zone, the Homestead), plus
the parameterized dungeon generator `makeDungeonMap()`, each its own
function in `js/zones.js` that would need its own transcription the
